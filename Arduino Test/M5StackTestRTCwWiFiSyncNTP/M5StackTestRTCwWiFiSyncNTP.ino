...
char timeStrbuff1[64];
char timeStrbuff2[64];

void flushTime(){
    M5.Rtc.GetTime(&RTCtime); //Gets the time in the real-time clock. 
    M5.Rtc.GetDate(&RTCDate);
    sprintf(timeStrbuff1,"%d/%02d/%02d",  //Stores real-time time and date data to timeStrbuff.  
                        RTCDate.Year,RTCDate.Month,RTCDate.Date);
    sprintf(timeStrbuff2,"%02d:%02d:%02d",  //Stores real-time time and date data to timeStrbuff.   
                        RTCtime.Hours,RTCtime.Minutes,RTCtime.Seconds);
    M5.lcd.setCursor(10,100); //Move the cursor position to (x,y).  
    M5.Lcd.println(timeStrbuff1);  //Output the contents of. 
    M5.lcd.setCursor(10,140);
    M5.Lcd.println(timeStrbuff2);
}
